...
// Component contains input fields for section 2 of a SLMP Delete Request
// Responsible for sending input information to be saved in the Flask backend.
// Allows for user attachment, Endorse and Reject options

// prop with id of request to be endorsed
type SignEndorseProp = {
  id: string;
};

// current date to be used in the form
...
const SignEndorse: React.FC<SignEndorseProp> = ({ id }) => {
  ...
  // handles submission of form, for both endorse and reject decisions
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      if (endorseData["Action"] == "endorse") {
        const data = await submitForm(
          "/submitslmp/delete/section2",
          endorseData
        );
        ...
        const data = await submitForm(
          "/submitslmp/delete/section2-reject",
          endorseData
        );
  ...
  // handles change to form input fields to update current data state
  const handleChange = (
    e: React.ChangeEvent<
  ...
  return (
    <div style={{ margin: "1rem" }}>
      ...
      <form onSubmit={handleSubmit} className="row g-3">
        ...
        <div className="col-md-4">
          <label htmlFor="selectUninstalled" className="form-label">
          ...
          <div className="form-check">
            <input
              ...
              required
            />
            <label className="form-check-label" htmlFor="isUninstalled">
              Yes
            </label>
          </div>
          <div className="form-check">
            <input
              ...
              required
            />
            <label className="form-check-label" htmlFor="notUninstalled">
              No
            </label>
          </div>
        </div>
        <div className="col-md-4">
          ...
          <div className="form-check">
            <input
              ...
              required
            />
            <label className="form-check-label" htmlFor="isTracked">
              Yes
            </label>
          </div>
          <div className="form-check">
            <input
              ...
              required
            />
            <label className="form-check-label" htmlFor="notTracked">
              No
            </label>
          </div>
        </div>
